...
def get_processor_runnable():
    ...
    # Processor agent needs status update tool
    tools = [update_customer_status]
    llm = create_agent_llm("processor", tools=tools, temperature=0.3)
...
async def processor_node(
    ...
    # Add context about what was logged
    ...
    if "messages" not in updates:
        updates["messages"] = []
    updates["messages"].append(log_context)
    updates["messages"].append(response)

    # Mark as complete - end the conversation
...
def processor_node_sync(
    state: ConversationState, config: RunnableConfig
) -> Dict[str, Any]:
    """Synchronous wrapper for processor_node."""
    import asyncio

    return asyncio.run(processor_node(state, config))
